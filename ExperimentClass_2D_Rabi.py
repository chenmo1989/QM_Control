class EH_Rabi:
	...
	Methods:
		update_tPath
		update_str_datetime
		qubit_freq_vs_dc_flux(self, poly_param, ham_param, dc_flux_sweep, qubit_index, n_avg, cd_time, tPath = None, f_str_datetime = None, simulate_flag = False, simulation_len = 1000)
	"""
	...
	def qubit_freq_vs_dc_flux(self, machine, dc_flux_sweep, qubit_if_sweep, qubit_index, n_avg, cd_time, ham_param = None, poly_param = None, simulate_flag=False, simulation_len=1000, plot_flag=True):
		"""
		...
		for dc_index, dc_value in enumerate(dc_flux_sweep): # sweep over all dc fluxes
			...
			if plot_flag == True:
				machine, I_tmp, Q_tmp = self.exp1D.res_freq(machine, res_freq_sweep, n_avg, cd_time, simulate_flag=simulate_flag, simulation_len=simulation_len, fig = fig)
			else:
				machine, I_tmp, Q_tmp = self.exp1D.res_freq(machine, res_freq_sweep, n_avg, cd_time, simulate_flag=simulate_flag, simulation_len=simulation_len)
			...
			if plot_flag == True:
				machine, I_tmp, Q_tmp = self.exp1D.qubit_freq(machine, qubit_freq_sweep, qubit_index, qubit_index, ff_amp=0.0, n_avg=n_avg, cd_time=cd_time, simulate_flag=simulate_flag, simulation_len=simulation_len, fig = fig)
			else:
				machine, I_tmp, Q_tmp = self.exp1D.qubit_freq(machine, qubit_freq_sweep, qubit_index, qubit_index, ff_amp=0.0, n_avg=n_avg, cd_time=cd_time, simulate_flag=simulate_flag, simulation_len=simulation_len)
	...
	def qubit_freq_vs_fast_flux_slow(self, machine, ff_sweep_abs, qubit_if_sweep, qubit_index, n_avg, cd_time, ff_to_dc_ratio = None, poly_param = None, simulate_flag=False, simulation_len=1000, plot_flag=True):
		"""
		...
		for ff_index, ff_value in enumerate(ff_sweep):  # sweep over all fast fluxes
			...
			if plot_flag == True:
				machine, I_tmp, Q_tmp = self.exp1D.qubit_freq(machine, qubit_freq_sweep, qubit_index, ff_amp = ff_value, n_avg = 1E3, cd_time = 10E3, simulate_flag = simulate_flag, simulation_len = simulation_len, fig = fig)
			else:
				machine, I_tmp, Q_tmp = self.exp1D.qubit_freq(machine, qubit_freq_sweep, qubit_index, ff_amp = ff_value, n_avg = 1E3, cd_time = 10E3, simulate_flag = simulate_flag, simulation_len = simulation_len)
	...
	def qubit_freq_fast_flux_subroutine(self, machine, ff_sweep_rel, qubit_freq_est_sweep, qubit_if_sweep, qubit_index, n_avg = 1E3, cd_time = 10E3, simulate_flag = False, simulation_len = 1000, fig = None):
		"""
	...
	def qubit_freq_vs_fast_flux(self, machine, qubit_freq_sweep, qubit_if_sweep, qubit_index, n_avg, cd_time, poly_param = None, simulate_flag=False, simulation_len=1000, plot_flag=True):
		"""
		2D qubit spectroscopy experiment vs fast flux
